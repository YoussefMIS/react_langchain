...
def find_tool_by_name(tools: list[Tool | BaseTool], name: str) -> Tool | BaseTool:
    for tool in tools:
        if tool.name == name:
            return tool
    raise ValueError(f"Tool with name {name} not found in the provided tools list.")
...
if __name__ == "__main__":
    ...
    llm = ChatOllama(model="qwen3:0.6b", temperature=0, stop=["\nObservation","Observation"],
